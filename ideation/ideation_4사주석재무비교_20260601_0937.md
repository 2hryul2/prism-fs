...
### P2 (후속)
- **⑤ 주석↔재무제표 정합 참조**: account_nm↔주석번호 후보를 임베딩으로 제시, 확정은 사용자. 숫자 자동 일치판정 금지
- **⑥ 이상치/누락 플래그**: 결정론 룰(부호 반전·N배 급변·셀 누락·단위 불일치·대응 없음). "확인 필요" 톤
- **⑦ 회계정책(주기) diff**: 동일 주제 주석 회사/기간 병치. 요약은 LLM 옵트인 시에만
...
| 🔍 주석 검색·비교 | fs_div 셀렉터 + 주기/서술형 필터 | 샘플 + "백엔드 연동" 배지 |
