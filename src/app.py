...
def tokenize_korean(text: str) -> list:
    """공백·문장부호 기반 단순 토크나이저. KoNLPy 가 있으면 형태소 분석 권장."""
    return [t for t in _KOREAN_TOKEN_RE.findall(text or "") if len(t) > 1]
...
def match_query_in_notes(q_emb, q_text: str, notes_with_emb: list) -> Optional[dict]:
    ...
    - cos+BM25(가중 0.7/0.3) → argmax → lexical_hit → keep → confidence
    - lexical_hit: 질의 토큰이 매치 제목에 글자 그대로(부분문자열) 존재하는가.
      모든 오답은 BM25=0(질의어가 제목에 부재) → lexical_hit 로 정/오답 구분.
    ...
    Args:
        ...
        notes_with_emb: "embedding" 키를 가진 note dict 리스트
    Returns:
        {note_no,title,page_start,page_end,score,confidence,lexical_hit,keep} 또는
        후보 없음(빈 목록) 시 None.
    """
    if not notes_with_emb:
        return None

    cos_scores = np.array([cosine(q_emb, n["embedding"]) for n in notes_with_emb])

    if USE_BM25 and _HAS_BM25:
        corpus = [n.get("tokens") or tokenize_korean(n["title"]) for n in notes_with_emb]
        bm25 = BM25Okapi(corpus)
        q_tokens = tokenize_korean(q_text)
    ...
    q_tokens = tokenize_korean(q_text)
    lexical_hit = any(qt in hit["title"] for qt in q_tokens)
